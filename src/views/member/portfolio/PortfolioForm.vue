...
const formRules = reactive({
  ...
  portfolioTagType: [
    { required: true, message: "作品标签类型不能为空", trigger: "change" }
  ],
  designerStyleType: [
    { required: true, message: "作品风格类型不能为空", trigger: "change" }
  ],
  content: [
    {
      validator: (rule: any, value: string, callback: any) => validateCountWords(rule,value,callback,500),
      trigger: 'blur'
    }
  ]
});
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      ...
      formData.value = {
        ...data,
        portfolioTagType: data.portfolioTagType ? data.portfolioTagType.split(",").map((item) => Number(item)) : [],
        designerStyleType: data.designerStyleType ? data.designerStyleType.split(",").map((item) => Number(item)) : []
      };
    } finally {
